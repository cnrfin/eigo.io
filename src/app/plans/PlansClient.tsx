...
export default function PlansPage() {
  ...
  // Check trial window. Logged-out visitors (and crawlers) are treated as
  // trial-eligible, so they see the full price AND the discounted "with trial"
  // price (making it clear the discount exists) and the page stays indexable.
  // We only settle this once auth has resolved so an active-countdown user
  // doesn't flash the wrong state first.
  useEffect(() => {
    if (!session?.access_token) { if (!loading) setPriceTier('trial'); return }
  ...
  const handleCheckout = useCallback(async (plan: 'light' | 'standard' | 'test') => {
    // Logged out: prompt sign-up / login instead of hitting Stripe.
  ...
  return (
    <main className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: 'var(--bg)' }}>
      ...
      {/* ─── Back button (only for logged-in users; logged-out visitors have no
             dashboard, so we don't expose a link into it) ─── */}
      {user && (
            ...
            >
              ...
              >
                ...
                <Squircle asChild cornerRadius={10} cornerSmoothing={0.8}>
                  <button
                    onClick={() => { if (user) { window.location.href = '/dashboard?tab=booking' } else { setShowAuth(true) } }}
